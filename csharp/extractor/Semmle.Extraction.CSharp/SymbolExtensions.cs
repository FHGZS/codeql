...
namespace Semmle.Extraction.CSharp
{
    ...
    static class SymbolExtensions
    {
        ...
        public static void BuildTypeId(this ITypeSymbol type, Context cx, TextWriter trapFile, bool addBaseType, ISymbol symbolBeingDefined)
        {
            using (cx.StackGuard)
            {
                switch (type.TypeKind)
                {
                    ...
                    case TypeKind.Error:
                        var named = (INamedTypeSymbol)type;
                        named.BuildNamedTypeId(cx, trapFile, addBaseType, symbolBeingDefined);
                        return;
                    case TypeKind.Pointer:
                        var ptr = (IPointerTypeSymbol)type;
        ...
        static void BuildNamedTypeId(this INamedTypeSymbol named, Context cx, TextWriter trapFile, bool addBaseType, ISymbol symbolBeingDefined)
        {
            if (named.IsTupleType)
            {
                trapFile.Write('(');
            ...
            if (addBaseType && named.GetNonObjectBaseType(cx) is INamedTypeSymbol @base)
            {
                trapFile.Write(" : ");
